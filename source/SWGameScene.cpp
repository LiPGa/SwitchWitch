...
//        unitsInBoard.push_back(getUnitType(unitSubType, unitColor));
    ...
    for (auto child : children) {
        ...
        shared_ptr<Unit> unit = Unit::alloc(Unit::Color::RED, basicAttackVec, specialAttackVec, Vec2(0,-1));
...
//            squareNode->setPosition((Vec2(squarePosition.x, squarePosition.y) * SQUARE_SIZE) + Vec2::ONE * (SQUARE_SIZE / 2));
            ...
            if (unitColor == "red") {
                c = Unit::Color::RED;
            } else if (unitColor == "blue") {
                c = Unit::Color::BLUE;
            } else {
                c = Unit::Color::GREEN;
            }
            shared_ptr<Unit> unit = Unit::alloc(c, unitTemplate->getBasicAttack(),unitTemplate->getSpecialAttack(), unitDirection);
            sq->setUnit(unit);
            ...
            if (_currentState == CONFIRM_SWAP)
            {
                ...
                for (shared_ptr<Square> attackedSquare: _attacked_squares) {
                    ...
                    if (attackedSquare->getUnit()->isSpecial()) {
                        if (attackedSquare->getUnit()->getDirection() == Vec2(0, 1)) {
                            attackedSquare->getViewNode()->setTexture(_textures.at("special_up_square"));
                        }
                        else if (attackedSquare->getUnit()->getDirection() == Vec2(0, -1)) {
                            attackedSquare->getViewNode()->setTexture(_textures.at("special_down_square"));
                        }
                        else if (attackedSquare->getUnit()->getDirection() == Vec2(1, 0)) {
                            attackedSquare->getViewNode()->setTexture(_textures.at("special_right_square"));
                        }
                        else {
                            attackedSquare->getViewNode()->setTexture(_textures.at("special_left_square"));
                        }
                    }
                    else {
