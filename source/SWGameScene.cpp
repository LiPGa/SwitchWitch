...
void GameScene::setLevel(shared_ptr<cugl::JsonValue> levelJSON) {
    ...
    _board->setViewNode(_boardNode);
    // Set the view of the board.
    _squareSizeAdjustedForScale = _defaultSquareSize * min(_scale.width, _scale.height) * (min((float)_maxBoardHeight / (float)_level->getNumberOfRows(), (float)_maxBoardWidth / (float)_level->getNumberOfColumns())) * 1.3;
